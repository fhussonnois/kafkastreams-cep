...
import com.github.fhuz.kafka.streams.cep.nfa.buffer.impl.StackEventKey.StateKey;
import com.github.fhuz.kafka.streams.cep.nfa.buffer.impl.TimedKeyValue.Pointer;
...
public class KVSharedVersionedBuffer<K , V> implements SharedVersionedBuffer<K, V> {
    ...
    public void branch(Stage<K, V> stage, Event<K, V> event, DeweyVersion version) {
        ...
        Pointer pointer = new Pointer(version, key);
    ...
    public Sequence<K, V> peek(Stage<K, V> stage, Event<K, V> event, DeweyVersion version, boolean remove) {
        Pointer pointer = new Pointer(version, newStackEventKey(stage, event));
